...
const checkAndParseTxtFileToCsv = async (csvFilePath, csvToTxtFilePath) => {
  ...
  await unlinkFileIfExist(fullCsvToTxtFilePath);
  parseCsv(fullCsvFilePath, fullCsvToTxtFilePath);
};

const unlinkFileIfExist = async (filePath) => {
  try {
...
const parseCsv = (csvFilePath, fullCsvToTxtFilePath) => {
  return csv().fromFile(csvFilePath).on('data', async (data) => {
...
if (require.main === module) {
  ...
  module.exports = { checkAndParseTxtFileToCsv };
}
